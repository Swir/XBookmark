/* SWIR 10.29 STABLE — SYMBOL SAFE NICKS + ACK ROUTE GUARD
 * Stable composition promoted from 10.29 beta.
 * Exact nickname punctuation, per-socket ACK routing, bounded friend queue,
 * fresh snapshot merge, clean Friends panel, MIX 8/8 and ICE dark text.
 */
(()=>{try{
if(window.__SWIR_STABLE1029_BOOT)return;window.__SWIR_STABLE1029_BOOT=1;
...
(async()=>{
 await add(CDN+BASE106+'/swir-beta-106.js?s1029='+Date.now());
 if(!await wait())throw new Error('base did not become ready');
 ...
 window.SWIR_CLOUD_VERSION='10.29 STABLE';paint();document.addEventListener('click',e=>{if(e.target?.closest?.('#btnConfig'))setTimeout(paint,30)},true);
 window.SWIR_STABLE1029={version:'10.29 STABLE',diagnostics:()=>({nick:window.SWIR_NICK_INTEGRITY1029?.diagnostics?.(),friends:window.SWIR_FRIENDS_SYMBOL1029?.diagnostics?.(),queue:window.SWIR_QUEUE1023?.diagnostics?.(),routeGuard:window.SWIR_ACK_ROUTE_GUARD1028?.diagnostics?.(),merge:window.SWIR_SNAPSHOT_MERGE1029?.diagnostics?.(),rooms:window.SWIR_ROOMS_PANEL1029?.diagnostics?.(),mix:window.SWIR_MIX1019?.diagnostics?.()})};
 console.log('SWIR 10.29 STABLE ready');
})().catch(e=>{console.error('SWIR 10.29 stable bootstrap',e);alert('SWIR 10.29 STABLE: błąd startu. Odśwież stronę i wybierz poprzednią wersję Stable.')});
}catch(e){console.error('SWIR 10.29 stable bootstrap',e)}})();
