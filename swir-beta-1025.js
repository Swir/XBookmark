 ...
 * Frozen 10.6 UI + 10.22 ACK core + 10.23 Queue Watch + 10.24 trace
 * + conservative fresh per-connection 159 merge (15s TTL).
 * Confirmed 10.19 MIX stays untouched. Honour Probe stays removed.
 */
(()=>{try{
if(window.__SWIR_BETA1025_BOOT)return;window.__SWIR_BETA1025_BOOT=1;
...
function paint(){try{const p=document.getElementById('configPanel');if(!p)return;p.querySelectorAll('.swir99-ver').forEach(x=>x.textContent='v10.25');p.querySelectorAll('.swir99-badge').forEach(x=>x.remove());const sub=p.querySelector('.sw10-summary,.swir-summary,[data-swir-summary]');if(sub)sub.textContent='SWIR MOD';}catch(e){}}
(async()=>{
 await add(CDN+BASE106+'/swir-beta-106.js?b1025='+Date.now());
 ...
 await add(CDN+selfRef+'/swir-snapshot-merge-1025.js?v='+Date.now());
 ...
 window.SWIR_CLOUD_VERSION='10.25 BETA';
 paint();document.addEventListener('click',e=>{if(e.target?.closest?.('#btnConfig'))setTimeout(paint,30)},true);
 window.SWIR_BETA1025={version:'10.25 BETA',diagnostics:()=>({friends:window.SWIR_FRIENDS_ACK1022?.diagnostics?.(),queue:window.SWIR_QUEUE1023?.diagnostics?.(),snapshot:window.SWIR_SNAPSHOT_TRACE1024?.diagnostics?.(),merge:window.SWIR_SNAPSHOT_MERGE1025?.diagnostics?.(),rooms:window.SWIR_ROOMS_PANEL1023?.diagnostics?.(),mix:window.SWIR_MIX1019?.diagnostics?.()})};
 console.log('SWIR 10.25 BETA ready — Snapshot Merge');
})().catch(e=>{console.error('SWIR 10.25 bootstrap',e);alert('SWIR 10.25 BETA: błąd startu. Odśwież stronę i wróć do 10.24/10.23 BETA.')});
}catch(e){console.error('SWIR 10.25 bootstrap',e)}})();