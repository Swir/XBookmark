 ...
 * 10.25 Snapshot Merge + diagnostic verification that ACK 8/4 and the
 * following 85 travel through the same live connection. No timing changes.
 * MIX 10.19 stays frozen. Honour Probe stays removed. STABLE untouched.
 */
(()=>{try{
if(window.__SWIR_BETA1026_BOOT)return;window.__SWIR_BETA1026_BOOT=1;
...
function paint(){try{const p=document.getElementById('configPanel');if(!p)return;p.querySelectorAll('.swir99-ver').forEach(x=>x.textContent='v10.26');p.querySelectorAll('.swir99-badge').forEach(x=>x.remove());const sub=p.querySelector('.sw10-summary,.swir-summary,[data-swir-summary]');if(sub)sub.textContent='SWIR MOD';}catch(e){}}
(async()=>{
 await add(CDN+BASE106+'/swir-beta-106.js?b1026='+Date.now());
 ...
 await add(CDN+selfRef+'/swir-ack-route-trace-1026.js?v='+Date.now());
 ...
 window.SWIR_CLOUD_VERSION='10.26 BETA';paint();document.addEventListener('click',e=>{if(e.target?.closest?.('#btnConfig'))setTimeout(paint,30)},true);
 window.SWIR_BETA1026={version:'10.26 BETA',diagnostics:()=>({friends:window.SWIR_FRIENDS_ACK1022?.diagnostics?.(),queue:window.SWIR_QUEUE1023?.diagnostics?.(),route:window.SWIR_ACK_ROUTE_TRACE1026?.diagnostics?.(),snapshot:window.SWIR_SNAPSHOT_TRACE1024?.diagnostics?.(),merge:window.SWIR_SNAPSHOT_MERGE1025?.diagnostics?.(),rooms:window.SWIR_ROOMS_PANEL1023?.diagnostics?.(),mix:window.SWIR_MIX1019?.diagnostics?.()})};
 console.log('SWIR 10.26 BETA ready — ACK Route Trace');
})().catch(e=>{console.error('SWIR 10.26 bootstrap',e);alert('SWIR 10.26 BETA: błąd startu. Odśwież stronę i wróć do 10.25 BETA.')});
}catch(e){console.error('SWIR 10.26 bootstrap',e)}})();