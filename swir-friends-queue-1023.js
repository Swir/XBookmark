...
function inspect(){const c=core();if(!c)return null;let diag=null;try{diag=c.diagnostics?.()||null}catch(e){}const jobs=c.jobs?.()||[];const serverCount=Number(diag?.serverFriends??0),fresh=!!diag?.serverFresh;
 if(lastServerCount>=0&&fresh&&lastServerFresh&&serverCount<lastServerCount){const d=load();d.drops=Array.isArray(d.drops)?d.drops:[];d.drops.push({ts:now(),from:lastServerCount,to:serverCount,primary:diag?.primary||null,active:diag?.active||''});if(d.drops.length>30)d.drops=d.drops.slice(-30);save(d);push('snapshot-drop',{from:lastServerCount,to:serverCount})}
 lastServerCount=serverCount;lastServerFresh=fresh;return{c,diag,jobs}}
...
function tick(){try{const x=inspect();if(!x)return;const {c,diag,jobs}=x;if(busy(jobs)){lastDecision='busy';return}
 ...
 const unresolved=jobs.some(j=>!['CONFIRMED','ID_CONFLICT'].includes(j?.state));if(unresolved&&now()-lastAutoRefresh>15000){lastAutoRefresh=now();lastDecision='refresh85';push('refresh85',{serverFriends:Number(diag?.serverFriends||0)});c.refresh?.();return}
 lastDecision='idle';
...
function diagnostics(){const x=inspect(),d=load();const out={version:'10.23 QUEUE WATCH',decision:lastDecision,retries:[...retry.entries()].map(([nick,v])=>({nick,...v})),snapshotDrops:d.drops||[],history:(d.history||[]).slice(-25),core:x?.diag||null};console.log('[SWIR 10.23] queue watch',out);return out}
