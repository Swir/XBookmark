 ...
 * 10.26 diagnostics + narrow routing correction: every code 85 sent shortly
 * after ACK 8/4 is pinned to the WebSocket that delivered that ACK.
 * No timing changes. MIX 10.19 frozen. Honour Probe removed. STABLE untouched.
 */
(()=>{try{
if(window.__SWIR_BETA1027_BOOT)return;window.__SWIR_BETA1027_BOOT=1;
...
const ROUTE_PIN_REF='091eb553f5403a232ecc7c341dfcd6f4e9ff8df6';
...
function paint(){try{const p=document.getElementById('configPanel');if(!p)return;p.querySelectorAll('.swir99-ver').forEach(x=>x.textContent='v10.27');p.querySelectorAll('.swir99-badge').forEach(x=>x.remove());const sub=p.querySelector('.sw10-summary,.swir-summary,[data-swir-summary]');if(sub)sub.textContent='SWIR MOD';}catch(e){}}
(async()=>{
 await add(CDN+BASE106+'/swir-beta-106.js?b1027='+Date.now());
 ...
 await add(CDN+ROUTE_PIN_REF+'/swir-ack-route-pin-1027.js?v='+Date.now());
 ...
 window.SWIR_CLOUD_VERSION='10.27 BETA';paint();document.addEventListener('click',e=>{if(e.target?.closest?.('#btnConfig'))setTimeout(paint,30)},true);
 window.SWIR_BETA1027={version:'10.27 BETA',diagnostics:()=>({friends:window.SWIR_FRIENDS_ACK1022?.diagnostics?.(),queue:window.SWIR_QUEUE1023?.diagnostics?.(),routeTrace:window.SWIR_ACK_ROUTE_TRACE1026?.diagnostics?.(),routePin:window.SWIR_ACK_ROUTE_PIN1027?.diagnostics?.(),snapshot:window.SWIR_SNAPSHOT_TRACE1024?.diagnostics?.(),merge:window.SWIR_SNAPSHOT_MERGE1025?.diagnostics?.(),rooms:window.SWIR_ROOMS_PANEL1023?.diagnostics?.(),mix:window.SWIR_MIX1019?.diagnostics?.()})};
 console.log('SWIR 10.27 BETA ready — ACK Route Pin');
})().catch(e=>{console.error('SWIR 10.27 bootstrap',e);alert('SWIR 10.27 BETA: błąd startu. Odśwież stronę i wróć do 10.26 BETA.')});
}catch(e){console.error('SWIR 10.27 bootstrap',e)}})();